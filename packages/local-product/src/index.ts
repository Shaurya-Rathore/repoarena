...
type LocalPublicTask = Readonly<
	AgentVisibleTask & {
		...
		source: AgentVisibleTask["source"] &
			Pick<Task["source"], "issue_url" | "pr_url" | "discovery">;
	}
>;
...
const taskQuerySchema = querySchema.extend({
	source: z.string().max(40).default(""),
	status: z.string().max(40).default(""),
	min_quality: z.coerce.number().min(0).max(1).default(0),
});
...
const safeError = (response: ServerResponse, error: unknown) => {
	...
	json(response, known || invalid ? 400 : 500, {
		schema: "repoarena.local-error/v1",
		error: {
			code: known
				? error.code
				: invalid
			...
			message: known
				? error.message
				: invalid
					? "Local API input failed validation."
...
const publicRun = (run: PersistedRun): PersistedRun => ({
	...run,
	attempts: run.attempts.map((attempt) => ({
		...attempt,
		artifacts: attempt.artifacts.filter(
			(artifact) => artifact.visibility !== "EVALUATOR_PRIVATE",
		),
	})),
});
const runSummary = (run: PersistedRun) => {
	...
	for (const attempt of run.attempts) {
		...
		group.attempts++;
		group.solved += attempt.evaluation.outcome === "SOLVED" ? 1 : 0;
		group.duration_ms += attempt.duration_ms;
		group.cost_micros += attempt.cost.micros ?? 0;
...
export function createLocalProductServer(
	...
	const optimizationDirectory = join(
		options.root,
		".repoarena",
		"state",
		"optimizations",
	);
	const readTasks = async (): Promise<LocalPublicTask[]> =>
		Promise.all(
			(await files(taskDirectory, /\.(json|ya?ml)$/i)).map(async (file) => {
				const raw = await readFile(join(taskDirectory, file), "utf8");
				const task = taskSchema.parse(
					file.endsWith(".json") ? JSON.parse(raw) : parse(raw),
				);
				...
				return {
					...
					source: {
						...visible.source,
						issue_url: task.source.issue_url,
						pr_url: task.source.pr_url,
						discovery: task.source.discovery,
					},
					validation: task.validation,
					provenance: task.provenance,
				};
	...
	const readRuns = async (): Promise<PersistedRun[]> => {
		const values = await Promise.all(
			(await files(runsDirectory, /\.json$/)).map((file) =>
				loadRun(join(runsDirectory, file)).catch(() => null),
			),
		);
		const unique = new Map(
			values
				.filter((item): item is PersistedRun => item !== null)
				.map((run) => [run.id, publicRun(run)]),
		);
		...
		for (const file of await files(optimizationDirectory, /\.json$/)) {
			try {
				const value = JSON.parse(
					await readFile(join(optimizationDirectory, file), "utf8"),
				) as OptimizationRun;
				if (value.schema === "repoarena.optimization-run/v1")
					values.push(value);
			} catch {
				/* corrupt history is omitted from lists, never fabricated */
			}
		}
		return values.sort(
			(a, b) =>
	...
	const readReadinessHistory = async (): Promise<ReadinessReport[]> => {
		...
		for (const file of await files(readinessDirectory, /\.json$/)) {
			try {
				const value = JSON.parse(
					await readFile(join(readinessDirectory, file), "utf8"),
				) as ReadinessReport;
	...
	const mutationAllowed = (request: IncomingMessage) => {
		...
		const validOrigin =
			...
			(origin === address?.url || origin.startsWith("http://localhost:"));
		return (
			validOrigin &&
			request.headers["x-repoarena-csrf"] === csrfToken &&
		...
		try {
			...
			if (url.pathname === "/health")
				return json(response, 200, {
					status: "ok",
					schema: "repoarena.local-health/v1",
				});
			...
			if (request.method === "GET" && path === "/repository") {
				...
				try {
					identity = await new GitRepository(options.root).identity();
				} catch {
					/* onboarding state */
				}
				let branch = "unavailable";
				try {
					branch = (await readFile(join(options.root, ".git", "HEAD"), "utf8"))
						.trim()
						.replace("ref: refs/heads/", "");
				} catch {
					/* onboarding state */
				}
				const initialized = await stat(
					join(options.root, ".repoarena", "config.yaml"),
				)
					.then(() => true)
					.catch(() => false);
				return json(response, 200, {
					data: {
						schema: "repoarena.local-repository/v1",
						name: basename(options.root),
						root_label: basename(options.root),
						initialized,
						branch,
			...
			if (request.method === "GET" && path === "/configuration")
				return json(response, 200, { data: await loadConfig(options.root) });
			if (request.method === "PUT" && path === "/configuration") {
				if (!mutationAllowed(request))
					return json(response, 403, {
						error: {
							code: "FORBIDDEN",
							message: "Same-origin CSRF validation failed.",
						},
					});
				return json(response, 200, {
					data: await writeConfigAtomic(options.root, await readBody(request)),
				});
			}
			if (request.method === "GET" && path === "/tasks") {
				...
				const all = (await readTasks()).filter(
					(task) =>
						`${task.id} ${task.title} ${task.metadata.tags.join(" ")}`
							.toLowerCase()
							.includes(query.search.toLowerCase()) &&
						(!query.source || task.source.type === query.source) &&
						(!query.status || task.validation.status === query.status) &&
						(task.metadata.quality_score ?? 0) >= query.min_quality,
				);
				const items = all
			...
			if (request.method === "GET" && taskMatch?.[1]) {
				const task = (await readTasks()).find(
			...
			if (request.method === "GET" && path === "/runs") {
				...
				const all = (await readRuns()).filter(
					(run) =>
						(!query.status || run.status === query.status) &&
						(!query.agent ||
							run.attempts.some(
								(attempt) => attempt.agent.id === query.agent,
							)) &&
						(!query.model ||
							run.attempts.some(
								(attempt) => attempt.agent.model === query.model,
							)) &&
						(!query.from || run.created_at >= query.from) &&
						(!query.to || run.created_at <= query.to),
				);
				return json(response, 200, {
					data: {
						items: all
			...
			if (request.method === "GET" && runMatch?.[1]) {
				const run = (await readRuns()).find(
					(item) => item.id === decodeURIComponent(runMatch[1] ?? ""),
				);
				return run
			...
			if (request.method === "GET" && attemptMatch?.[1]) {
				const attempt = (await readRuns())
					.flatMap((run) => run.attempts)
					.find(
						(item) => item.id === decodeURIComponent(attemptMatch[1] ?? ""),
					);
				return attempt
					? json(response, 200, { data: attempt })
					: json(response, 404, {
							error: {
			...
			if (request.method === "GET" && path === "/readiness") {
				try {
					const value = JSON.parse(
						await readFile(join(readinessDirectory, "latest.json"), "utf8"),
					) as ReadinessReport;
					return json(response, 200, { data: value });
				} catch {
					return json(response, 200, { data: null });
				}
			}
			if (request.method === "GET" && path === "/readiness/history") {
				const query = querySchema.parse(Object.fromEntries(url.searchParams));
				const all = await readReadinessHistory();
				return json(response, 200, {
					data: {
			...
			if (request.method === "POST" && path === "/readiness") {
				...
				const report = await assessRepository(options.root);
				await atomicJson(join(readinessDirectory, `${report.id}.json`), report);
				await atomicJson(join(readinessDirectory, "latest.json"), report);
				return json(response, 201, { data: report });
			}
			if (request.method === "GET" && path === "/optimizations") {
				const query = querySchema.parse(Object.fromEntries(url.searchParams));
				const all = await readOptimizations();
				return json(response, 200, {
					data: {
						items: all.slice(query.offset, query.offset + query.limit),
						total: all.length,
			...
			if (request.method === "GET" && optimizationMatch?.[1]) {
				const value = (await readOptimizations()).find(
					(item) => item.id === decodeURIComponent(optimizationMatch[1] ?? ""),
				);
				return value
			...
			if (request.method === "POST" && path === "/optimizations") {
				...
				if (!options.optimizerExecutor)
					return json(response, 409, {
						error: {
							code: "OPTIMIZER_UNAVAILABLE",
							message: "This server was not configured with a trial executor.",
						},
					});
				...
				return json(response, 201, { data: run });
			}
			...
			if (request.method === "GET" && profileMatch?.[1]) {
				...
				response.setHeader(
					"content-disposition",
					`attachment; filename="repoarena-profile-${value.id}.yaml"`,
				);
				response.end(exportRecommendedProfile(value));
				return;
			}
			return json(response, 404, {
				error: { code: "NOT_FOUND", message: "Local API route was not found." },
			});
		} catch (error) {
			...
			new Promise((resolve, reject) => {
				...
				server.listen(options.port ?? 4177, host, () => {
					...
					if (!bound || typeof bound === "string")
						return reject(new Error("local server address unavailable"));
					address = {
						host,
						port: bound.port,
