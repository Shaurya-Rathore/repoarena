...
it("explains missing tests, environment, guidance, services, monorepo and generated code", async () => {
	const root = await fixture({ ".git/HEAD": "main", "README.md": "docs", "package.json": JSON.stringify({ scripts: { start: "node app" } }), "packages/a/package.json": "{}", "packages/b/package.json": "{}", "packages/c/package.json": "{}", "packages/d/package.json": "{}", "migrations/001.sql": "create table example(id int);", "src/generated/client.ts": "generated" });
	const report = await assessRepository(root, { now: "2026-01-01T00:00:00.000Z" });
	expect(report.findings.map((item) => item.id)).toEqual(expect.arrayContaining(["tests.missing", "environment.unspecified", "agent-guidance.missing", "dependencies.services", "complexity.monorepo", "complexity.generated"]));
