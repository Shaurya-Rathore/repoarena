...
const fixture = async (files: Record<string, string>) => {
	const root = await mkdtemp(join(tmpdir(), "repoarena-ready-"));
	roots.push(root);
...
it("scores a reproducible repository deterministically", async () => {
	const root = await fixture({
		...
		".repoarena/tasks/task.yaml": "task",
		"AGENTS.md": "Run pnpm verify.",
		...
		"src/example.test.ts": "test('ok',()=>{})",
	});
	const first = await assessRepository(root, {
		now: "2026-01-01T00:00:00.000Z",
	});
	const second = await assessRepository(root, {
		now: "2026-01-01T00:00:00.000Z",
	});
...
it("explains missing tests, environment, guidance, services, monorepo and generated code", async () => {
	const root = await fixture({
		...
		"package.json": JSON.stringify({
			scripts: {
				start: "node app",
				"test:slow": "curl https://example.invalid/fixture",
				"test:flaky": "runner --retry 3",
			},
		}),
	...
	expect(
		report.findings.every((item) => item.evidence && item.recommendation),
	).toBe(true);
});
it("persists a readiness snapshot atomically without disturbing stale temporary files", async () => {
	...
	await writeFile(
		join(destination, "..", ".readiness-interrupted.tmp"),
		"partial",
	);
