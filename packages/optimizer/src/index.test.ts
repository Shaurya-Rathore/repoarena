...
it("enforces budgets, reuses provenance cache, compares baseline and exports a profile", async () => {
	...
	expect(first.trials).toHaveLength(4); expect(first.recommendation?.candidate.model).toBe("fast"); expect(first.recommendation?.candidate.reasoning).toBe("low"); expect(first.recommendation?.delta.success_rate).toBeGreaterThanOrEqual(0); expect(first.holdout_task_ids).toEqual(["holdout"]); expect(exportRecommendedProfile(first)).toContain("repoarena.profile/v1");
