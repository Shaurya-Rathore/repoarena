...
| Runner lifecycle and local state | IN_PROGRESS | `packages/runner-core`, `packages/git`, `packages/integrity`; runner lifecycle tests | `pnpm --filter @repoarena/runner-core test:unit && pnpm --filter @repoarena/runner-core typecheck` | Connected isolated agent/public/private lifecycle captures the patch before a second evaluator workspace materializes private data. Add sandbox/adapter integration, persistence, reports and E2E matrix. |
