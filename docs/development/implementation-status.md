...
| Runner lifecycle and local state | IN_PROGRESS | `packages/runner-core`, `packages/integrity`; runner and integrity tests | focused package tests | Argv-safe verification and deterministic Git-based integrity analysis exist. Add isolated state machine, private evaluator sequencing, persistence, and engine integration. |
