...
| Runner lifecycle and local state | IN_PROGRESS | `packages/runner-core`, `packages/run-store`, `packages/git`, `packages/integrity`; lifecycle and persistence tests | focused runner/run-store tests | Connected isolated agent/public/private lifecycle and atomic public-only run persistence pass. Add adapter orchestration, recovery policy, reports and E2E matrix. |
...
| Reporting | IN_PROGRESS | `packages/reporter`; public reporter/XSS tests | `pnpm --filter @repoarena/reporter test:unit && pnpm --filter @repoarena/reporter typecheck` | Terminal, versioned JSON, standalone HTML, and JUnit consume the canonical public run model and agree on stored statistics/cost. Add connected CLI/report E2E and XML parser validation. |
