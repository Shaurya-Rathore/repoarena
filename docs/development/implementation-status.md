...
| Runner lifecycle and local state | IN_PROGRESS | `packages/runner-core`, `packages/benchmark-engine`, `packages/run-store`, `packages/git`, `packages/integrity`; lifecycle/concurrency/persistence/CLI E2E tests | `pnpm verify`; focused runner, benchmark-engine, run-store and CLI tests | Connected lifecycle, bounded clean repetitions, deterministic ordering, incremental atomic persistence, real aggregation, pricing snapshots, CLI E2E and test-deletion rejection pass. Add full adapter usage propagation, resumability/retries, artifact manifests and adversarial matrix. |
