...
| Runner lifecycle and local state | IN_PROGRESS | `packages/runner-core`, `packages/benchmark-engine`, `packages/run-store`, `packages/git`, `packages/integrity`; lifecycle/concurrency/persistence/CLI E2E tests | `pnpm --filter @repoarena/runner-core test:unit`; `pnpm --filter @repoarena/benchmark-engine test:unit`; `pnpm --filter repoarena test:unit` | Connected lifecycle now validates transitions through private verification, captures untracked/allowlisted artifacts, redacts secrets from verification and patches, performs bounded clean repetitions, incrementally persists results, propagates observed adapter usage, and passes CLI E2E. Resumability/retries and the full adversarial matrix remain. |
...
| Evaluation, metrics and statistics | IN_PROGRESS | `packages/evaluator`, `packages/statistics`, runner/benchmark E2E; decision/privacy/anti-cheating/aggregation tests | `pnpm verify` | Behavior decision, safe private projection, integrity precedence, pass@k, cost/churn aggregation, concurrent repetitions, redacted patch evidence and CLI E2E pass. Add baseline regression inventory and complete fake-agent/security matrix. |
| Pricing and cost accounting | IN_PROGRESS | `packages/pricing`, `packages/benchmark-engine`; catalog/cost snapshot and benchmark integration tests | `pnpm --filter @repoarena/pricing test:unit && pnpm --filter @repoarena/benchmark-engine test:unit` | Versioned effective-date lookup, explicit availability, immutable snapshots, and benchmark persistence provenance pass. Add provider-reported usage propagation and historical report reload test. |
