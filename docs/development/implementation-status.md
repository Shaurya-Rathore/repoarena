...
| Local product UI | IN_PROGRESS | `packages/local-product`; typed API/UI integration tests | local-product unit suite and typecheck | Localhost-only API and responsive application cover overview, tasks, runs, attempts/diffs, comparison, readiness, optimizer history, and validated configuration. CLI wiring and browser E2E remain. |
